pipeline {
  ...
  post {
    always {
      sh script: 'mail -s "Jenkins Build ${currentResult}: Job ${JOB_NAME}" -r "AlcaDB Team <alcadb.user@cern.ch>" -c "101pritam@gmail.com" physics.pritam@gmail.com <<< "${currentResult}"', label: "Sending post-build email"
